...
static YGSize RNDateTimePickerMeasure(YGNodeRef node, float width, YGMeasureMode widthMode, float height, YGMeasureMode heightMode)
{
    ...
    dispatch_sync(dispatch_get_main_queue(), ^{
        ...
        if (shadowPickerView.timeZoneName) {
            ...
            if (timeZone != nil) {
                ...
                RCTLogWarn(@"'%@' does not exist in NSTimeZone.knownTimeZoneNames. Falling back to localTimeZone=%@",
                          shadowPickerView.timeZoneName,
...
{
    if (self = [super init]) {
        YGNodeSetMeasureFunc(self.yogaNode, RNDateTimePickerMeasure);
    }
    return self;
}
